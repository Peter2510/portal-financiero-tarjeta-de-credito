...
const crearUsuarioYTarjetaCredito = async (req, res) => {
    const { nombre, correo_electronico, direccion, documento_identificacion, telefono, id_rol, a2f_activo, pin, nombre_usuario,
         id_tipo_tarjeta, notificar_uso, limite_credito, id_entidad_proveedor } = req.body;

    const t = await sequelize.transaction();

    try {
        // Verificar si el usuario ya existe
        let usuario = await Usuario.findOne({ where: { correo_electronico }, transaction: t });
        ...
        const tipoTarjeta = await TipoTarjeta.findByPk(id_tipo_tarjeta, { transaction: t });
        if (!tipoTarjeta) {
            await t.rollback();
            return res.status(404).json({ ok: false, mensaje: 'No se encontró el tipo de tarjeta' });
        }

        // Verificar entidad proveedora
        const entidadProveedor = await EntidadProveedor.findByPk(id_entidad_proveedor, { transaction: t });
        if (!entidadProveedor) {
            await t.rollback();
            return res.status(404).json({ ok: false, mensaje: 'No se encontró la entidad proveedora' });
        }

        // Crear nombre de la tarjeta
        const nombre_tarjeta = `${usuario.nombre_usuario}.${tipoTarjeta.tipo}@${entidadProveedor.entidad}.com`;

        // Crear la tarjeta de crédito
        await TarjetaCredito.create({
            id: uuidv4(),
            id_tipo_tarjeta,
            fecha_creacion: new Date(),
            notificar_uso,
        ...
        return res.status(201).json({
            ok: true,
            mensaje: 'Usuario y tarjeta de crédito creados correctamente'
        });
...
function generarNumeroTarjeta() {
    return Math.floor(100000 + Math.random() * 900000).toString(); // Asegurando que sea un string
}

function generarCVV() {
    return Math.floor(100 + Math.random() * 900).toString(); // Asegurando que sea un string
}
